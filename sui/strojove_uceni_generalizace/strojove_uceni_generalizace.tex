...
\begin{compactitem}
    ...
    \item Formálně; $J$ je objektivní funkce, $\theta$ jsou parametry modelu, $D$ je dataset (s označenými daty -- učení s učitelem), $loss$ je chybová funkce, $\lambda$ je funkce, která počítá regularizaci vah a $\beta$ je funkce, která počítá regularizaci aktivací.
    $$
